...
const StyledTitleDoughFlavor = styled(DialogTitle)(() => ({
	padding: 4,
	color: 'grey',
	fontSize: 14,
}))
...
type DoughFlavorType = {
	_id: string
	type: string
	name: string
	flavors: { doughFlavor: string }[]
}
const DoughFlavor: React.FC = () => {
	...
	const [dataFlavor, setDataFlavor] = useState<DoughFlavorType[]>([])
	...
	const handleDoughFlavorChoice = (choice: string) => {
								...
								{activeFlavor.flavors.length > 0 ? (
									activeFlavor.flavors.map(variant => (
										<StyledLi
											...
											onClick={() => handleDoughFlavorChoice(variant.flavor)}
										>
