...
const StyledTitleExtraCreamFlavor = styled(DialogTitle)(() => ({
	padding: 4,
	color: 'grey',
	fontSize: 14,
}))
...
type ExtraCreamFlavorType = {
	_id: string
	type: string
	name: string
	flavors: { extraCreamFlavor: string }[]
}
const ExtraCreamFlavor: React.FC = () => {
	...
	const [dataFlavor, setDataFlavor] = useState<ExtraCreamFlavorType[]>([])
	...
	const handleExtraCreamFlavorChoice = (choice: string) => {
		setIngredients((prevIngredients: Ingredients) => ({
			...prevIngredients,
			extraCreamFlavor: choice,
		}))
								...
								{activeFlavor.flavors.length > 0 ? (
									activeFlavor.flavors.map(variant => (
										<StyledLi
											...
											onClick={() =>
												handleExtraCreamFlavorChoice(variant.flavor)
											}
										>
...
export default ExtraCreamFlavor
