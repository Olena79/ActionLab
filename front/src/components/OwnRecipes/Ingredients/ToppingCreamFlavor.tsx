...
const StyledTitleCreamFlavor = styled(DialogTitle)(() => ({
	padding: 4,
	color: 'grey',
	fontSize: 14,
}))
...
type CreamFlavorType = {
	_id: string
	type: string
	name: string
	flavors: { toppingCreamFlavor: string }[]
}
const CreamFlavor: React.FC = () => {
	...
	const [dataFlavor, setDataFlavor] = useState<CreamFlavorType[]>([])
	...
	const handleCreamFlavorChoice = (choice: string) => {
		setIngredients((prevIngredients: Ingredients) => ({
			...prevIngredients,
			toppingCreamFlavor: choice,
		}))
								...
								{activeFlavor.flavors.length > 0 ? (
									activeFlavor.flavors.map(variant => (
										<StyledLi
											...
											onClick={() => handleCreamFlavorChoice(variant.flavor)}
										>
